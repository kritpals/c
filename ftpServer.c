...
void * doftp(void* newSd);
int writen(int sd, char *ptr, int size);
int readn(int sd, char *ptr, int size);
pthread_t *dataTransferThread;
...
main() {
    ...
    while (1 == 1) {
        ...
        printf("server: return from accept, socket for this ftp: %d\n",
                newsd);
        dataTransferThread = (pthread_t*) malloc(sizeof (pthread_t));
        int returnValue = pthread_create(dataTransferThread, NULL, &doftp, (void*) newsd);
        if (returnValue) {
            printf("Failed to spawn a new thread.");
            fflush(stdout);
        } else {
            printf("New thread started successfully.");
            fflush(stdout);
        }
        pthread_detach(*dataTransferThread);
        //     if ( (pid=fork()) == 0) {
