 ...
 *Simple dns response sniffer in C++ .Open raw socket and start sniffing  udp packet and IP protocol 17.
 * If Proto is 17 then check for source port 53.
 * Check the no of answer received and parse it accordingly.
 *Every 60 seconds print the data used by each Domain 
...
void process_packet(unsigned char* buffer, int size) {
    ...
    if (strcasecmp(dMAC, "xx:xx:xx:xx:xx:xx") == 0) { // If this Client packet comes.
        //        printf("Storing data\n");
