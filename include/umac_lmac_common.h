...
*/
#define ATH_SET_VAP_BSSID_MASK(bssid_mask)			((bssid_mask)[5] &= ~((ATH_BCBUF-1) & 0x0f))
#define ATH_GET_VAP_ID(bssid, hwbssid, id)			do { id = (((bssid)[5]  - hwbssid[5]) & 0x0f); } while(0)
#define ATH_SET_VAP_BSSID(bssid, hwbssid, id)                       \
			do {                                        \
				if (id) {                           \
					(bssid)[5] = hwbssid[5] + (id & 0x0f);  \
				}                                   \
			} while(0)
#endif
